...
/** Shared 401/403/429 mapping for {@link safeCall} and composite tool abort. */
export function toolResultForAuthAndRateLimit(
  e: HttpLikeError,
  mode: AuthMode,
): CallToolResult | null {
  if (e.status === 401) {
...
export function compositeToolAbortOnHttpError(
  ...
  for (const r of results) {
    ...
    const tr = toolResultForAuthAndRateLimit(toHttpLike(r.reason), mode);
    if (tr) return tr;
  }
  return null;
}
